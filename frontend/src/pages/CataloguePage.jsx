...
const categories = ["All", "RICE", "PULSES", "FLOUR", "SUGAR", "SPICES", "DRY_GROCERIES", "MASALAS"];
...
export default function CataloguePage({ category: initialCategory }) {
  ...
  return <section className="section-shell py-14 sm:py-20">
    ...
    <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{products.map((product) => <article key={product.id} className="premium-card flex flex-col overflow-hidden p-6"><div className="flex h-36 items-center justify-center rounded-2xl bg-brand-forest text-center font-display text-2xl text-brand-gold">{product.imageUrl ? <img src={product.imageUrl} alt="" className="h-full w-full object-cover" /> : product.name}</div><p className="mt-5 text-xs font-bold uppercase tracking-widest text-brand-gold">{pretty(product.category)}</p><h2 className="mt-2 font-display text-2xl">{product.name}</h2>{product.sku && <p className="mt-1 text-xs font-semibold text-brand-emerald/55">SKU: {product.sku}</p>}<p className="mt-3 flex-1 text-sm leading-6 text-brand-emerald/70">{product.shortDescription || product.description}</p><p className="mt-4 text-xs font-semibold text-brand-emerald/60">{product.packSize ? `${product.packSize} ${product.packUnit || ""}` : "Pack size on request"} · {product.b2bEligible ? "B2B price available to eligible accounts" : "Commercial availability on request"}</p><div className="mt-5 flex gap-3"><Link to={`/products/${product.slug}`} className="flex-1 rounded-full border border-brand-forest px-4 py-2 text-center text-xs font-bold uppercase tracking-wider">Details</Link><button onClick={() => add(product)} className="flex-1 rounded-full bg-brand-forest px-4 py-2 text-xs font-bold uppercase tracking-wider text-brand-sand">Add to quote</button></div></article>)}</div>
  </section>;
}
